...
- **Spend tiles show "No data"** — OpenUsage found no qualifying Codex usage in Codex or OpenCode logs from the last 30 days (Codex usage made through pi is counted on the [Pi](pi.md) card). If your Codex home lives somewhere custom, set `CODEX_HOME` so both the Codex CLI and OpenUsage look in the same place.
